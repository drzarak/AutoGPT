...
def test_validate_url():
    ...
    with pytest.raises(ValueError):
        validate_url("http://[::ffff:127.0.0.1]", [])  # IPv4 loopback mapped to IPv6
    with pytest.raises(ValueError):
        validate_url(
            "http://[::ffff:169.254.169.254]", []
        )  # IPv4 link-local mapped to IPv6
    with pytest.raises(ValueError):
        validate_url("http://[::ffff:10.0.0.1]", [])  # IPv4 private mapped to IPv6
    with pytest.raises(ValueError):
        validate_url("http://[::ffff:192.168.1.1]", [])  # IPv4 private mapped to IPv6
